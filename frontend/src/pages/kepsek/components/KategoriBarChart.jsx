...
export default function KategoriBarChart({ stats }) {
  ...
  return (
    <svg viewBox={`0 0 ${width} ${height}`} style={{ width: '100%', maxWidth: '640px', height: 'auto', fontFamily: 'inherit' }}>
      <line x1={padLeft} y1={padTop} x2={padLeft} y2={padTop + chartH} stroke="#cbd5e0" strokeWidth="1.5" />
      <line x1={padLeft} y1={padTop + chartH} x2={padLeft + chartW} y2={padTop + chartH} stroke="#cbd5e0" strokeWidth="1.5" />
      ...
      {Array.from({ length: steps + 1 }, (_, i) => {
        ...
        return (
          <g key={i}>
            ...
            <text x={padLeft - 8} y={y + 4} fontSize="11" fill="#94a3b8" textAnchor="end">
              {val}
            </text>
          </g>
        );
      ...
      {data.map((d, i) => {
        ...
        return (
          <g key={d.key}>
            ...
            <text x={x + barW / 2} y={y - 8} fontSize="13" fontWeight="700" fill="#2d3748" textAnchor="middle">
              {d.value}
            </text>
            <text x={x + barW / 2} y={padTop + chartH + 20} fontSize="12" fill="#4a5568" textAnchor="middle">
              {d.label}
            </text>
          </g>
        );
